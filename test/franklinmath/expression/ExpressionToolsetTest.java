...
public class ExpressionToolsetTest {
    ...
    @Before
    public void setUp() throws ExecutionException, java.io.IOException {
        //try to load in in the system function information
        FunctionInformation functionInformation = new FunctionInformation("functions.xml");
    ...
    public void testNumberAdd() throws ExpressionException, ExecutionException, ParseException {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            strInput = String.format("%d + %d", num1.longValue(), num2.longValue());
            resultExpr = ProcessString(strInput);
    ...
    public void testNumberMultiplyAndDivide() throws ExpressionException, ExecutionException, ParseException {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            String mulStr = String.format("%d * %d", num1.longValue(), num2.longValue());
            String divStr = String.format("%d / %d", num1.longValue(), num2.longValue());
    ...
    public void testSimpleSymbolicPowerArithmatic() throws Exception {
        ...
        for (int i = 0; i < 500; i++) {
            ...
            int numExtraMultiplies = random.nextInt(8) + 1;
            String execString = "var^" + exponent;
            for (int j = 0; j < numExtraMultiplies; j++) {
                execString = execString + "*var^" + exponent;
            }
            resultExpr = ProcessString(execString);
            expectedPower = new Power(new Factor("var", true));
            expectedPower = expectedPower.AppendFactor(new Factor(exponent * (numExtraMultiplies + 1)));
            assertEquals(new Expression(new Term(expectedPower), TermOperator.NONE), resultExpr);
        }
    }
    ...
    public void testFlattenTermNumbers() throws ExpressionException, ExecutionException {
        ...
        Term resultTerm = expressionToolset.FlattenTerm(term, 0);
        ...
        resultTerm = expressionToolset.FlattenTerm(term, 0);
    ...
    public void testFlattenPower() throws ExpressionException, ExecutionException {
        ...
        Power resultPower = expressionToolset.FlattenPower(power, 0);
        ...
        resultPower = expressionToolset.FlattenPower(power, 0);
